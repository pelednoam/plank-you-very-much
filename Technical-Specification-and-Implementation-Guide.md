...
| ------------- | ----------------------------------------------- | ---------------------------- | - | --------- |
| App           | **Next.js 15 (React 19) + TypeScript**          | Robust full-stack framework  |   |           |
| Styling       | **Tailwind CSS 4**                              | Utility-first, quick theming |   |           |
| Charts        | **Recharts**                                    | Lightweight SVG charts       |   |           |
| State         | **Zustand 5**                                   | Minimal, scalable store      |   |           |
| Forms         | **React Hook Form 7**                           | Type-safe validation         |   |           |
| Date          | **Day.js**                                      | Small footprint              |   |           |
| Persistence   | **IndexedDB via idb**                           | Offline PWA support          |   |           |
| Notifications | Web Push + service worker                       | (via Next.js PWA plugin)     |   |           |
...
| PWA           | **@ducanh2912/next-pwa** (or similar)           | Next.js PWA integration      |   |           |
...
/plankyou
  ...
  │   ├─ hooks/
  │   ├─ lib/          # Utilities, API clients, storage adapters
  │   ├─ store/        # Zustand stores
  │   ├─ types/        # TypeScript interfaces/types
  │   └─ globals.css
  ├─ cypress/          # E2E Tests
  ├─ .github/workflows/
  └─ next.config.mjs   # Next.js config (inc. PWA)
```
(Note: `tests/` replaced by `cypress/` convention. `src/pages`, `src/router`, `src/serviceWorker.ts`, `src/index.tsx` removed/replaced by Next.js App Router conventions.)
...
1. **Register the app** at [https://dev.fitbit.com](https://dev.fitbit.com) → Manage Apps. • Callback: `${process.env.NEXT_PUBLIC_FITBIT_REDIRECT_URI}` (e.g., `https://plankyou.app/oauth/fitbit/callback`). • Save `client_id`, generate `client_secret` (store only in serverless function / backend).
2. **Frontend flow (**`@/features/settings/components/FitbitConnectButton.tsx`**)
   ```ts
   ...
   ```
3. **Callback handler (Next.js API Route or Server Action)** `/app/oauth/fitbit/callback/route.ts`
   ```ts
   // POST https://api.fitbit.com/oauth2/token
   // grant_type=authorization_code&code=XXX&client_id=...&redirect_uri=...
   // Authorization: Basic base64(client_id:client_secret)
   ```
   Store `access_token` (8 h) & `refresh_token` (persists) encrypted or securely associated with user profile.
4. **Scheduled Job / Serverless Function** pulls `/activities/date/{date}.json`, `/sleep/date/{date}.json`, etc., transforms to `FitbitDaily` model, updates store/DB.
5. **Token refresh** with `refresh_token` via API route/server action; handle 401 by revoking & prompting reconnect.

*Libraries:* `simple-oauth2` (server), `useSWR` or React Query (client).
